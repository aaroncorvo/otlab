...
`phase1-sensor-sim-cross-pi.pcap` — captured 2026-05-06 with `tcpdump -i eth0 -w` on `l3-mon-01`. Shows 4 Modbus TCP request/response pairs from `l1-plc-01` (10.20.30.47) reading the sensor-sim slave on `l3-mon-01:5020`. Includes the TCP three-way handshake and a clean disconnect.
